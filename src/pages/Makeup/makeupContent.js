...
export default function MakeupContent({item}) {
  ...
  const handleLike = async () => {
    if (Like == LikeIcon) {
      await fetch(`http://localhost:5000/api/post/${item._id}/like`, { method: "PUT", headers: { 'Content-Type': "application/Json", token: accessToken } })
  ...
  useEffect(() => {
    const getuser = async () => {
      try {
        const res = await axios.get(`http://localhost:5000/api/user/post/user/details/${item.user}`)
        setuser(res.data);
      } catch (error) {
  ...
  return (
    <div className='insidemakeupcontent' onClick={()=>handleClick(item)}>
          ...
          <p style={{ textAlign: 'start', width: "96%", marginLeft: 20,fontSize:"13px", marginTop: 0 }}>{item.title?.slice(0,50)}</p>
