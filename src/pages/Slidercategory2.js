...
function Slidercategory2() {
  ...
  useEffect(() => {
    const getProducts = async ()=>{
        try {
            const res = await axios.get(`https://api.aavelance.com/api/products/getallProduct?category=Men's Fashion`)
  ...
  useEffect(() => {
    const getPosts = async () => {
      try {
        const res = await axios.get(`https://api.aavelance.com/api/post/getallpost?category=Men's Fashion`)
        setPosts(res.data);
      } catch (error) {
