...
const Product = () => {
  ...
  const handleCreate = async()=>{
    try {
      await fetch('https://api.aavelance.com/api/products/reviews/product', {method: 'PUT',
  ...
  useEffect(() => {
    const getPosts = async () => {
      try {
        const res = await axios.get(`https://api.aavelance.com/api/post/getallpost?category=${product?.categories}&subcategories=${product?.subcategories}`)
        setPosts(res.data);
        return () => {};
      } catch (error) {
    ...
    <Container>
      ...
      <div className="ProductWrapper">
        ...
        <div className="ProductInfoContainer">
          ...
          <div >
            <textarea style={{width:"0px" , height:"0px"}} security={true} ref={textAreaRef} value={`https://www.aavelance.com/product/find/${product?._id}?affid=${userid}`}></textarea>
          </div>
