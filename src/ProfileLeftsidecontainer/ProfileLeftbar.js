...
export default function ProfileLeftbar() {
  ...
  useEffect(() => {
    const getFollowing = async () => {
      try {
        const res = await axios.get(`http://api.aavelance.com/api/post/following/${id}`);
  ...
  const handleFollow = async () => {
    if (Follow === "Follow") {
      await fetch(`http://api.aavelance.com/api/user/following/${id}`, { method: 'PUT', headers: { 'Content-Type': "application/JSON", token: accessToken }, body: JSON.stringify({ user: `${userDetails?.currentUser?.others?._id}` }) })
