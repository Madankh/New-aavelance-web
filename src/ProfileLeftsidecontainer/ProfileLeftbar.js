...
export default function ProfileLeftbar() {
  ...
  useEffect(() => {
    const getuser = async () => {
      try {
        const res = await axios.get(`https://api.aavelance.com/api/user/post/user/details/${id}`)
        setuser(res.data);
      } catch (error) {
  ...
  useEffect(() => {
    const getFollowing = async () => {
      try {
        const res = await axios.get(`https://api.aavelance.com/api/post/following/${id}`);
  ...
  const handleFollow = async () => {
    if (Follow === "Follow") {
      await fetch(`https://api.aavelance.com/api/user/following/${id}`, { method: 'PUT', headers: { 'Content-Type': "application/JSON", token: accessToken }, body: JSON.stringify({ user: `${userDetails?.currentUser?.others?._id}` }) })
