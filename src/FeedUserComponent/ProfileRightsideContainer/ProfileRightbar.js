...
export default function ProfileRightbar() {
  ...
  useEffect(() => {
    const getFollowing = async()=>{
      try {
        const res = await axios.get(`https://api.aavelance.com/api/post/followers/${id}`);
  ...
  useEffect(() => {
    const getuser = async()=>{
      try {
        const res  = await axios.get(`https://api.aavelance.com/api/user/all/user/${idforSuggest}`)
        setUsers(res.data);
      } catch (error) {
