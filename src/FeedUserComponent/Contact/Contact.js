...
export default function Contact() {
  ...
  const handleSearch = async (query) => {
    ...
    try {
      ...
      const { data } = await axios.get(`https://api.aavelance.com/api/auth/get/search/user?search=${search}`, config);
      setLoading(false);
  ...
  useEffect(() => {
    const getGroup = async () => {
      ...
      const { data } = await axios.get(`https://api.aavelance.com/api/chat/groups`, config);
      setCopy(data);
      setMyGroupChat(data)
    }
    getGroup();
  ...
  const handlesubmit = async () => {
    if (!groupName || groupMembers?.length <= 1) {
      ...
      try {
        await fetch("https://api.aavelance.com/api/chat/create/group", {
          method: "POST",
